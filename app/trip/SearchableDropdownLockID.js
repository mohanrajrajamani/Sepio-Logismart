...
import { ActivityIndicator, FlatList, StyleSheet, Text, ScrollView } from "react-native";
import Modal from 'react-native-modalbox';
import { widthPercentageToDP, heightPercentageToDP } from 'react-native-responsive-screen'

export default class searchableDropdownLockID extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    componentDidMount() {
        console.log("data : ", this.props.selectedLock)
    }
                    ...
                    <Container style={{ backgroundColor: '#fdfdfd', flex: 1 }}>
                        ...
                        <ScrollView style={{ paddingBottom: 20 }} >
                            <FlatList
                                ...
                                renderItem={({ item }) => (
                                    this.props.selectedLock !== undefined ? item.name !== this.props.selectedLock ?
                                        <ListItem onPress={() => this.selectItem(item)}>
                                            <Body>
                                                <Text style={{ color: "#263C88" }}>{item.itemName} {item.lock_vendor === 'jt' ? "(SP701)" : "(SP777)"}</Text>
                                            </Body>
                                        </ListItem> : null : <ListItem onPress={() => this.selectItem(item)}>
                                            <Body>
                                                <Text style={{ color: "#263C88" }}>{item.itemName} {item.lock_vendor === 'jt' ? "(SP701)" : "(SP777)"}</Text>
                                            </Body>
                                        </ListItem>
